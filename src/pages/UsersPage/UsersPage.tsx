    ...
    return (
        <AppLayout>
            <Table dataSource={users} className="userspage-table">
                <Column
                    title={UsersTableColumnTitle.Login}
                    dataIndex={UsersTableColumn.Login}
                    key={UsersTableColumn.Login}
                />
                <Column
                    title={UsersTableColumnTitle.Type}
                    dataIndex={UsersTableColumn.Type}
                    key={UsersTableColumn.Type}
                    render={(type) => <Tag color="green">{type}</Tag>}
                />
                <Column
                    title={UsersTableColumnTitle.SiteAdmin}
                    dataIndex={UsersTableColumn.SiteAdmin}
                    key={UsersTableColumn.SiteAdmin}
                    render={(isAdmin) => (
                        <Tag color={isAdmin ? "blue" : "red"}>
                            {isAdmin ? "Yes" : "No"}
                        </Tag>
                    )}
                />
                <Column
                    title="Action"
                    key="action"
                    render={(_, record: UsersListItem) => (
                        <Link to={record.login}>more</Link>
                    )}
                />
            </Table>
        </AppLayout>
